...
# Inputs from env: SG_NAME, APP_PORT, EC2_AMI_FILTER, EC2_INSTANCE_TYPE, EC2_INSTANCE_NAME, ASSOCIATE_EIP, YOUR_IP_CIDR
...
if [[ -z "${SG_ID}" || "${SG_ID}" == "None" ]]; then
  SG_ID=$(aws ec2 create-security-group --vpc-id "$VPC_ID" --group-name "$SG_NAME" --description "GHA EC2 SG" --query GroupId --output text)
  # SSH
